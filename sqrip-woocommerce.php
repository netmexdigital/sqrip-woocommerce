...
add_action('admin_enqueue_scripts', function ($hook_suffix) {
    ...
    wp_enqueue_style('sqrip-admin', plugins_url('css/sqrip-admin.css', __FILE__), '', '1.11.0-beta41');

    if (isset($_GET['section']) && $_GET['section'] == "sqrip") {
        // The reconcile-table row actions use dashicons (cancel / delete); make sure the
        // icon font is present. The "confirm" tick reuses the WooCommerce icon font, which
        // the WooCommerce settings screen already loads.
        wp_enqueue_style('dashicons');

        wp_enqueue_script('sqrip-admin', plugins_url('js/sqrip-admin.js', __FILE__), array('jquery', 'selectWoo'), '1.11.0-beta41', true);

        wp_enqueue_script('sqrip-avis', plugins_url('js/sqrip-avis.js', __FILE__), array('jquery', 'sqrip-admin'), '1.11.0-beta41', true);
        ...
        wp_localize_script('sqrip-admin', 'sqrip',
            array(
                ...
                'txt_avis_retry' => __('Please try again shortly or reload the page.', 'sqrip-swiss-qr-invoice'),
                'txt_avis_session_expired' => __('Your session has expired — that is why the reconciliation was refused. Please reload the page and try again.', 'sqrip-swiss-qr-invoice'),
                'txt_avis_reload_link' => __('Reload the page', 'sqrip-swiss-qr-invoice'),
                'txt_avis_confirm_paid' => __('Mark this order as paid? It moves to your configured status for completed orders.', 'sqrip-swiss-qr-invoice'),
                'txt_avis_confirm_cancel' => __('Cancel this order? Its status is set to "cancelled" and the order opens in a new tab.', 'sqrip-swiss-qr-invoice'),
                'txt_avis_confirm_forget' => __('Remove this payment from the list? It will not appear again.', 'sqrip-swiss-qr-invoice'),
                'txt_avis_action_failed' => __('The action could not be completed. Please reload the page and try again.', 'sqrip-swiss-qr-invoice'),
                ...
                'status_plugin_on' => __('sqrip is turned on.', 'sqrip-swiss-qr-invoice'),
                'status_plugin_off' => __('sqrip is turned off.', 'sqrip-swiss-qr-invoice'),
                'status_credits_none' => __('Not enough credits available. Please purchase credits here:', 'sqrip-swiss-qr-invoice'),
                'status_credits_enough' => __('Enough credits (%s) available.', 'sqrip-swiss-qr-invoice'),
                'status_credits_unknown' => __('Unable to fetch the available credits.', 'sqrip-swiss-qr-invoice'),
                'status_iban_ok' => __('(QR-)IBAN confirmed.', 'sqrip-swiss-qr-invoice'),
                'status_iban_no' => __('(QR-)IBAN not confirmed.', 'sqrip-swiss-qr-invoice'),
                'status_apikey_ok' => __('API key is correct and active.', 'sqrip-swiss-qr-invoice'),
                'status_server_ok' => __('Connected to the sqrip server.', 'sqrip-swiss-qr-invoice'),
                'status_server_no' => __('sqrip server not reachable.', 'sqrip-swiss-qr-invoice'),
                'status_error' => __('An error occurred. Could not request the status details.', 'sqrip-swiss-qr-invoice'),
                'status_avis_running' => __('Payment reconciliation via bank notifications checks every minute.', 'sqrip-swiss-qr-invoice'),
                'status_avis_unreachable' => __('Payment reconciliation via bank notifications: the service is not reachable.', 'sqrip-swiss-qr-invoice'),
                'status_avis_prefix' => __('Payment reconciliation via bank notifications:', 'sqrip-swiss-qr-invoice'),
                'avis_status_nonce' => wp_create_nonce('sqrip-avis'),
    ...
    if (is_object($screen) && (
        ...
        wp_enqueue_script('sqrip-order', plugins_url('js/sqrip-order.js', __FILE__), array('jquery'), '1.11.0-beta41', true);
        wp_enqueue_script('sqrip-refund', plugins_url('js/sqrip-refund.js', __FILE__), array('jquery'), '1.11.0-beta41', true);
    ...
    if (in_array($hook_suffix, ['user-edit.php', 'profile.php'])) {
        wp_enqueue_script('sqrip-customer-profile', plugins_url('js/sqrip-customer-profile.js', __FILE__), array('jquery'), '1.11.0-beta41', true);
...
function sqrip_enqueue_scripts()
{
    ...
    wp_enqueue_style('sqrip', plugins_url('css/sqrip-order.css', __FILE__), array(), '1.11.0-beta41');

    wp_enqueue_script('sqrip', plugins_url('js/sqrip-fe.js', __FILE__), array('jquery'), '1.11.0-beta41', true);
